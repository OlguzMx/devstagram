@extends('layouts.app')
@section('titulo') Inicia Sesión en DevStagram @endsection
@section('contenido')
<div class="md:flex md:justify-center md:gap-10 md:items-center">
    ...
    <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
        <form method="post" action="{{ route('login') }}">
            @csrf
            <!-- INICIO DE MENSAJE DE CREDENCIALES INCORRECTAS DEL USUARIO -->
            @if (session('mensaje'))
            <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">{{ session('mensaje') }}</p>
            @endif
            <!-- FIN DE MENSAJE DE CREDENCIALES INCORRECTAS DEL USUARIO -->
            <!-- INICIO DE EMAIL -->
            <div class="mb-5">
